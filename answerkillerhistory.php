...
while($row = mysqli_fetch_array($result)){$egg2 = $row['egg2'];}

$reg = $_POST["reg"];
if($reg){
    if($reg == "true" && $islogin == "true"){
        mysqli_query($conn,"UPDATE users SET egg2='true' WHERE id=N'$id';");
        echo("true");
    }
    else{echo("false");}
}
else{
    if($islogin == "true" && $egg2 == "true"){echo("yi解锁彩蛋——制作中,以后再来<a href=\"https://a.ordylan.cn/\">[返回首页]</a>!");}
